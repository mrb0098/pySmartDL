...
def url_fix(s, charset='utf-8'):
    ...
    :param charset: The target charset for the URL if the url was given as
                    unicode string. Default is 'utf-8'.
    :type charset: string
...
def is_HTTPRange_supported(url, timeout=15, proxies=None):
    '''
    ...
    :param proxies: Optional proxy dict (same format as SmartDL proxies parameter).
                    If None, uses whatever opener is currently installed globally.
    :type proxies: dict or None

    :rtype: bool
    '''
    url = url.replace(' ', '%20')
    fullsize = get_filesize(url, timeout=timeout, proxies=proxies)
    if not fullsize:
        return False
    ...
    # Use a custom opener with proxy if provided, otherwise use installed opener
    if proxies:
        opener = urllib.request.build_opener(urllib.request.ProxyHandler(proxies))
        urlObj = opener.open(req, timeout=timeout)
    else:
...
def get_filesize(url, timeout=15, proxies=None):
    '''
    ...
    try:
        if proxies:
            opener = urllib.request.build_opener(urllib.request.ProxyHandler(proxies))
            urlObj = opener.open(url, timeout=timeout)
        else:
...
def sizeof_human(num):
    '''
    Human-readable formatting for filesizes.
...
def time_human(duration, fmt_short=False, show_ms=False):
    '''
    Human-readable formatting for timing.
    ...
    :param fmt_short: Format as a short string
    :type fmt_short: bool
